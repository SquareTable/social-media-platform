...
const Conversations = ({navigation}) => {
    ...
    const ConversationItem = ({conversationId, isDirectMessage, members, conversationImageB64, conversationTitle, conversationNSFW, conversationNSFL, dateCreated, lastMessage, lastMessageDate, cryptographicNonce, conversationDescription, unreadsMessages}) => (
        <TouchableOpacity style={{borderColor: darkLight, borderWidth: 3, height: 100, borderRadius: 20, paddingTop: 5, paddingBottom: 5, paddingLeft: 2, paddingRight: 2, marginBottom: 10}} onPress={() => navigation.navigate("Chat", {conversationId: conversationId, conversationTitleSent: conversationTitle, cryptographicNonce: cryptographicNonce, conversationDescriptionSent: conversationDescription, isDirectMessage: isDirectMessage})}>
            <PostsHorizontalView>
                ...
                <PostsVerticalView style={{marginTop: 9, width: '55%'}}>
                    <View style={{height: 70, justifyContent: 'center', width: '100%'}}>
                        {conversationNSFW == true && (
                            <SubTitle numberOfLines={1} style={{fontSize: 20, marginBottom: 0, marginTop: 0, color: red, width: '100%'}}>{conversationTitle}</SubTitle>
                        )}
                        ...
                        {conversationNSFW == false && (
                            <View>
                                {conversationNSFL == false && (
                                    <SubTitle numberOfLines={1} style={{fontSize: 20, marginBottom: 0, marginTop: 0, width: '100%'}}>{conversationTitle}</SubTitle>
                                )}
                            </View>
                        )}
                        <SubTitle numberOfLines={1} style={{fontSize: 16, color: brand, marginBottom: 0, marginTop: 2, width: '100%'}}>{members} Members</SubTitle>
                    </View>
                </PostsVerticalView>
            </PostsHorizontalView>
            {unreadsMessages + extraUnreadsMessages.filter(x => x == conversationId).length !== 0 && (
                <View style={{position: 'absolute', right: 10, top: 10, fontWeight: 'bold'}}>
                    {unreadsMessages !== "" && (
                        <Text style={{backgroundColor: red, color: tertiary, borderRadius: 1000, aspectRatio: 1, width: 20, textAlign: 'center', position: 'absolute', fontWeight: 'bold', right: 5}}>
    ...
    const loadConversations = () => {
        ...
        handleMessage(null);
        const url = `https://nameless-dawn-41038.herokuapp.com/conversations/${_id}`;
    ...
    return(
        <>
            ...
            <KeyboardAvoidingWrapper>
                <StyledContainer style={{marginTop: -55}}>
                    <ScrollView>
                        ...
                        <SubTitle style={{marginBottom: 0, fontWeight: 'normal', fontSize: 12, textAlign: 'center', marginVertical: 2}}>Dm Users Online:</SubTitle>
                        {onlineUsersForShow.length == 0 && (
                            <TouchableOpacity style={{flexDirection: 'column', height: 80, width: 80, marginVertical: 10, borderRadius: 200, borderColor: darkLight, borderWidth: 3, alignItems: 'center', justifyContent: 'center'}}>
                                <Text style={{textAlign: 'center'}}>No one</Text>
                            </TouchableOpacity>
                        )}
                        {onlineUsersForShow.length !== 0 && (
                            <FlatList 
                                ...
                                horizontal={true}
                                style={{flexDirection: 'row', borderColor: darkLight, borderBottomWidth: 3}}
                                data={onlineUsersForShow}
                                keyExtractor={(item, index) => item.pubId}
                                                ...
                                                {item.imageKey !== "" && (
                                                    <View>
                                                        {onlineUsersImageB64s.findIndex(x => x.imageKey == item.imageKey) !== -1 ? (
                                                            <Image style={{height: 80, width: 80, borderRadius: 200, borderColor: darkLight, borderWidth: 3}} source={{uri: `data:image/jpg;base64,${onlineUsersImageB64s[onlineUsersImageB64s.findIndex(x => x.imageKey == item.imageKey)].imageB64}`}}/>
                                                        ) : (
                                                            <Image style={{height: 80, width: 80, borderRadius: 200, borderWidth: 3, borderColor: darkLight}}/>
                                                        )} 
                                                    </View>
                                                )}
                                                {item.imageKey == "" && (
                                                    <Image style={{height: 80, width: 80, borderRadius: 200, borderWidth: 3, borderColor: darkLight}} source={{uri: SocialSquareLogo_B64_png}}/>
                                                )}
                                            </TouchableOpacity>
                                            {item.displayName == "" && (
                                                <View style={{marginBottom: 5}}>
                                                    <Text style={{color: tertiary, textAlign: 'center'}}>@{item.name}</Text>
                                                </View>
                                            )}
                                            {item.displayName !== "" && (
                                                <View style={{marginBottom: 5}}>
                                                    <Text style={{color: brand, textAlign: 'center'}}>{item.displayName}</Text>
                                                    <Text style={{color: tertiary, textAlign: 'center'}}>@{item.name}</Text>
                                                </View>
                                            )}
                                        </View>
