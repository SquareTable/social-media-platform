...
import { AdvancedStylingVersion } from '../../components/StylingVersionsFile.js';
import {CredentialsContext} from '../../components/CredentialsContext.js';

const AdvancedStylingMenu = ({navigation, route}) => {
    const {dark, colors} = useTheme()
    const StatusBarHeight = Constants.statusBarHeight
    const [NamingNewStyle, setNamingNewStyle] = useState(true);
    const [message, handleMessage] = useState('')
    const [advancedStylingData, setAdvancedStylingData] = useState([])
    ...
    const [loadingNewCreation, setLoadingNewCreation] = useState(false)
    ...
    if (ableToRefresh == true) {
        ...
        async function getSimpleStylingData() {
            ...
            const tempData = data_parsed
            if (data != null) {
                for (let i = 0; i < tempData.length; i++) {
                    if (tempData[i].stylingType == 'simple') {
                        tempData.splice(i, 1)
                        i = i - 1
                    }
                }
                setAdvancedStylingData(tempData);
            }
        }
    ...
    const handleNewCreation = async (name) => {
        for (let i = 0; i < advancedStylingData.length; i++) {
            if (advancedStylingData[i].stylingType == 'advanced') {
                if (advancedStylingData[i].name == name) {
                    handleMessage('You already have an advanced style called ' + name + '. Please choose a different name.')
                    return
                }
            }
        }
        handleMessage('')
        const stylingData = await AsyncStorage.getItem('simpleStylingData')
        const stylingDataParsed = JSON.parse(stylingData)
        let tempData =  stylingDataParsed
        ...
        while (continueCheckIndexLoop == true) {
            ...
            for (let i = 0; i < advancedStylingData.length; i++) {
                if (advancedStylingData[i].indexNum == tempLength) {
                    tempLength += 1;
        ...
        tempData.push({
            ...
            indexNum: tempLength, 
            dark: true,
            stylingType: 'advanced',
            stylingVersion: AdvancedStylingVersion,
            colors: {
    ...
    useEffect(() => {
        async function getSimpleStylingData() {
            ...
            if (data != null) {
                ...
                setAdvancedStylingData(data_parsed);
        ...
        async function getSimpleStylingData() {
            ...
            if (amountOfStylesToGetUpdated == 0 || forceRefresh == true) {
                ...
                for (let i = 0; i < data_parsed.length; i++) {
                    try {
                        if (data_parsed[i].stylingType == 'advanced') {
                            if (data_parsed[i].stylingVersion != AdvancedStylingVersion) {
    ...
    const initiateStyleUpdate = () => {
        console.log('Initiating advanced style update');
        async function getSimpleStylingData() {
            let failedUpdates = [];
            ...
            for (let i = 0; i < data_parsed.length; i++) {
                if (data_parsed[i].stylingType == 'advanced') {
                    if (data_parsed[i].stylingVersion == 1) {
                        // PUT CODE IN HERE WHEN ADVANCED STYLING VERSION 2 IS MADE
                        let {name, indexNum, dark, colors} = data_parsed[i];
                        let { home, find, post, chat, profile } = colors;
                        console.log(name)
    ...
    const confirmDelete = async (name, indexNumber) => {
        for (let i = 0; i < advancedStylingData.length; i++) {
            console.log(advancedStylingData[i].indexNum)
            console.log(advancedStylingData)
            if (advancedStylingData[i].name == name) {
                let tempData = advancedStylingData;
    ...
    // CHANGE CHECKER TO SEE IF STYLE HAS CHANGED WHEN STYLES ARE ACTUALLY IMPLEMENTED
    const handlePress = (indexNum, forceStyle) => {
        if (AdvancedStylingVersion == advancedStylingData[indexNum].stylingVersion || forceStyle == true) {
            setRefreshAppStyling(true);
            ...
            if (colors.home != advancedStylingData[indexNum].colors.home) { //HERE
                setRefreshAppStyling(true);
                setTemp(temp => temp == 'abc' ? 'cba' : 'abc');
            }
        } else if (AdvancedStylingVersion < advancedStylingData[indexNum].stylingVersion) {
            let stylingVersionUsed = advancedStylingData[indexNum].stylingVersion;
        ...
        } else if (advancedStylingData[indexNum].stylingVersion == undefined || AdvancedStylingVersion > advancedStylingData[indexNum].stylingVersion) {
            let stylingVersionUsed = advancedStylingData[indexNum].stylingVersion;
            setVersionMismatchScreenVersion(stylingVersionUsed);
        ...
        <KeyboardAvoidingWrapper_NoScrollview style={{backgroundColor: colors.primary}}>
            <>
                <ChatScreen_Title style={{backgroundColor: colors.primary, borderWidth: 0}}>
                    ...
                    <TestText style={{textAlign: 'center', color: colors.tertiary}}>Advanced Styling</TestText>
                ...
                <ProfileOptionsView style={{backgroundColor: colors.primary}} viewHidden={versionMismatchScreenHidden}>
                    <View style={{flexDirection: 'column', justifyContent: 'center', alignItems: 'center'}}>
                        {versionMismatchScreenVersion != undefined ?
                            <>
                                <Text style={{color: colors.errorColor, fontSize: 18, fontWeight: 'bold', textAlign: 'center', marginHorizontal: 10}}>This style is version {versionMismatchScreenVersion || 'ERROR'} and this version of SocialSquare only supports advanced styles with version {AdvancedStylingVersion}.</Text>
                ...
                <ProfileOptionsView style={{backgroundColor: colors.primary, height: '80%'}} viewHidden={versionReleaseNotesHidden}>
                    <View style={{width: '100%', height: '100%'}}>
                        <ScrollView>
                            <Text style={{color: colors.tertiary, fontSize: 24, fontWeight: 'bold', textAlign: 'center'}}>{versionReleaseNotesVersion == 1 ? 'Release Notes for simple styling version ' + versionReleaseNotesVersion : null}</Text>
                            {versionReleaseNotesVersion == 1 ?
                                <>
                    ...
                    <FlatList
                            ...
                            <>
                                {amountOfStylesToGetUpdated != null && confirmUpdateScreenHidden == true && versionMismatchScreenHidden == true && versionReleaseNotesHidden == true && amountOfStylesToGetUpdated != 0 &&
                                    <TouchableOpacity onPress={() => {confirmUpdateScreenHidden == true ? setConfirmUpdateScreenHidden(false) : setConfirmUpdateScreenHidden(true)}} style={{padding: 5, backgroundColor: colors.primary, justifyContent: 'center', alignSelf: 'center', alignItems: 'center', borderColor: colors.borderColor, borderWidth: 3, borderRadius: 10, marginBottom: 10}}>
                                        <Text style={{fontSize: 20, fontWeight: 'bold', color: colors.tertiary, textAlign: 'center'}}>{displayName || 'Cannot find name'} has {amountOfStylesToGetUpdated} {amountOfStylesToGetUpdated == 1 ? 'style' : 'stylings'} that {amountOfStylesToGetUpdated == 1 ? 'needs' : 'need'} to be updated to V{AdvancedStylingVersion}</Text>
                        ...
                        ListEmptyComponent={
                            <View style={{flex: 1, justifyContent: 'center', alignItems: 'center', marginTop: '50%'}}>
                                <Text style={{color: colors.tertiary, fontSize: 20, fontWeight: 'bold', marginBottom: 20}}>There are no advanced stylings here.</Text>
                        ...
                        renderItem={({ item, index }) => ( 
                            <>
                                {confirmUpdateScreenHidden == true && versionMismatchScreenHidden == true && versionReleaseNotesHidden == true && item.stylingType == 'advanced' &&
                                    <View style={{borderColor: colors.borderColor, borderWidth: 3, flex: 1, paddingVertical: listOfDataGettingDeleted.includes(index) ? 0 : 15, flexDirection: 'row', borderTopWidth: index != 0 ? 1.5 : 3, borderBottomWidth: advancedStylingData.length - 1 == index ? 3 : 1.5, primaryColor: colors.primary}}>
                                            ...
                                            <>
                                                <TouchableOpacity style={{flexDirection: 'row', width: '100%'}} onPress={() => {handlePress(item.indexNum, false)}}>
                                                    ...
                                                    <TouchableOpacity onPress={() => {setListOfDataGettingDeleted(listOfDataGettingDeleted => [...listOfDataGettingDeleted, index])}} style={{position: 'absolute', right: 10, top: -7}}>
                                                        ...
                                                        {console.log('HI ' + (AppStylingContextState == (item.indexNum + 'ADVANCED')))}
...
export default AdvancedStylingMenu;